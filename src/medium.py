...
def common_elements(one: List[int], two: List[int]) -> List[int]:
    """ "take two lists as arguments, iterate through them and find the common numbers in both of them"""
    new = []
    for num in one:
...
def my_reverse(nums: List[int]) -> List[int]:
    left, right = 0, len(nums) - 1 # left and right vars are the first and last indices of the list nums
    while left < right: # while 0 is less than len(nums)
        nums[left], nums[right] = nums[right], nums[left] #swaps values at the left and right of the list nums
        left += 1 ## updates the values of left and right to move towards center of list
        right -= 1
    return nums